...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ) {
    ...
    unsigned char* I = (unsigned char*)mxGetPr(prhs[0]);//input image.
    int szPatch=(int)*mxGetPr(prhs[1]);//block size
    double facSift=*mxGetPr(prhs[2]);//overlapping factor
    ...
    plhs[0]=mxCreateNumericMatrix(numPatchX*numPatchY*16*numBin, 1, mxSINGLE_CLASS, mxREAL);
    float* y = (float*)mxGetPr(plhs[0]);//output pointer
...
//=========compute gradient of Image===================
    ...
    int* IgradOrient=(int*)mxMalloc(sizeof(int)*height*width); //0->numBin.
    for(i=0;i<height;i++){// compute the gradient of image.
        for (j=0;j<width;j++){
            if(i==height-1)
                ...
                if (j>0 && j< width-1)
                    dx=(((int)I[(j+1)*height+i+1]+(int)I[(j)*height+i+1]+(int)I[(j-1)*height+i+1])
                    -((int)I[(j+1)*height+i-1]+(int)I[(j)*height+i-1]+(int)I[(j-1)*height+i-1]))/3.0;
                else
                    dx=I[j*height+i+1]-I[j*height+i-1];
            }
            if(j==width-1)
                dy=(int)I[(width-1)*height+i]-(int)I[(width-2)*height+i];
            else if (j==0)
                dy=I[height+i]-I[i];
            else{
                if (i>0 && i< height-1)
                    dy=(((int)I[(j+1)*height+i+1]+(int)I[(j+1)*height+i]+(int)I[(j+1)*height+i-1])
                    -((int)I[(j-1)*height+i+1]+(int)I[(j-1)*height+i]+(int)I[(j-1)*height+i-1]))/3.0;
                else
                    dy=(int)I[(j+1)*height+i]-(int)I[(j-1)*height+i];
            }
...
//============compute weighted gradient orientation histogram=================
    float* orientHist=(float*)mxMalloc(sizeof(float)*numPatchX*numPatchY*16*numBin);
